...
public class ImageDicer extends Frame {
	...
	/**
	 * The createOps() method creates the image processing operations discussed
	 * in the column.
	 **/
	private void createOps() {
		...
		mOps.put("Invert blue", new LookupOp(
				new ShortLookupTable(0, blueInvert), null));
	 ...
	 * the user controls. This panel's size has to be accounted for later when
	 * we size the window to fit the image.
	 **/
	...
	/**
	 * When an image is loaded, the window is resized to accomodate the image
	 * size and the user controls.
	 **/
